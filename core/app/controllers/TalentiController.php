 ...
 */
class TalentiController extends \lithium\action\Controller {

	public function index(){
		$talenti = Talenti::find('all');
		return json_encode($talenti->data());
	}

	public function get($id){
		$talento = Talenti::find('all', ['conditions' => ['id_org' => $id]]);
		return json_encode($talento->data());
	}

	public function add(){
		$talento = Talenti::create();
		
		if($this->request->data && $talento->save($this->request->data)) {
			return json_encode($talento->data());
		}else{
			//error
		}
