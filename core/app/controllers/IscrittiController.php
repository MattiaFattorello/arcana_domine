...
class IscrittiController extends \app\controllers\CryptedController {
	...
	public function index(){
		...
		return json_encode($iscritti->data());
	}

	public function get($id){
		$iscritto = Iscritti::find('all', ['conditions' => ['id_org' => $id]]);
		return json_encode($iscritto->data());
	}

	public function add(){
		$iscritto = Iscritti::create();

		if($this->request->data && $iscritto->save($this->request->data)) {
			return json_encode($iscritto->data());
		}else{
			//error
		}
