 ...
 */
class PartecipantiController extends \lithium\action\Controller {

	public function index(){
		$partecipanti = Partecipanti::find('all');
		return json_encode($partecipanti->data());
	}

	public function get($id){
		$partecipante = Partecipanti::find('all', ['conditions' => ['id_org' => $id]]);
		return json_encode($partecipante->data());
	}

	public function add(){
		$partecipante = Partecipanti::create();
		
		if($this->request->data && $partecipante->save($this->request->data)) {
			return json_encode($partecipante->data());
		}else{
			//error
		}
