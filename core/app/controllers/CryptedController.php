  ...
  */
  ...
  private function getAuth(){
    if(isset($_REQUEST['user_id'])){
      $this->id   = openssl_decrypt(base64_decode($_REQUEST['user_id']), self::$encrypt_method, self::$key, 0, self::$iv);
    }

    if(isset($_REQUEST['user_role'])){
      $this->role = openssl_decrypt(base64_decode($_REQUEST['user_role']), self::$encrypt_method, self::$key, 0, self::$iv);
    }
