 ...
 */
class FeedbackController extends \lithium\action\Controller {

	public function index(){
		$feedbacks = Feedback::find('all');
		return json_encode($feedbacks->data());
	}

	public function get($id){
		$feedback = Feedback::find('all', ['conditions' => ['id_org' => $id]]);
		return json_encode($feedback->data());
	}

	public function add(){
		$feedback = Feedback::create();
		
		if($this->request->data && $feedback->save($this->request->data)) {
			return json_encode($feedback->data());
		}else{
			//error
		}
